...
-- Shortened format strings
battery_discharging_short_format = "B: %02.f:%02.f"
battery_charging_short_format = "B: C"
battery_ac_short_format = "B: AC"
battery_percent_short_format = " (%02.f/%02.f%%)"

power_discharging_short_format = "P:%6.f mW"
power_charging_short_format = "P: --"
...
function create_battery_widget(use_short_format, update_freq)
    local w = create_widget_helper(update_battery_state, update_freq or default_battery_update_freq)
    
    if use_short_format then
        metasettings[w].discharging_format = battery_discharging_short_format
        metasettings[w].charging_format = battery_charging_short_format
        metasettings[w].ac_format = battery_ac_short_format
        metasettings[w].percent_format = battery_percent_short_format
    else
...
function create_power_widget(use_short_format, update_freq)
    local w = create_widget_helper(update_power_state, update_freq or default_power_update_freq)

    if use_short_format then
        metasettings[w].discharging_format = power_discharging_short_format
        metasettings[w].charging_format = power_charging_short_format
    else
